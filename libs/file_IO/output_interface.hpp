...
public:
  ...
  virtual void WriteComplexImage(complex_tt **data, std::vector<ulong> shape, std::string label, 
                                 ...
                                 std::vector<float_tt> resolution=std::vector<float_tt>())=0;
  /*
  virtual void WriteRealFile(QSfMat &data, std::string label, std::vector<ulong> &indices=std::vector<ulong>(), 
                             std::map<std::string, double> &parameters=std::map<std::string, double>())=0;
  virtual void WriteComplexFile(QScMat &data, std::string label, std::vector<ulong> &indices=std::vector<ulong>(), 
                                std::map<std::string, double> &parameters=std::map<std::string, double>())=0;
  */
};
