...
public:
  ...
  virtual void WriteComplexImage(complex_tt **data, std::vector<ulong> shape, std::string label, 
                                 ...
                                 std::vector<float_tt> resolution=std::vector<float_tt>());
private:
  void WriteData(void **pix, bool is_complex, ulong dataSize, std::vector<ulong> shape, 
  ...
  /*
  void WriteRealImage(QSfMat &data, std::string label, std::vector<ulong> &position=std::vector<ulong>(), 
                      std::map<std::string, double> &parameters=std::map<std::string, double>());
  void WriteComplexImage(QScMat &data, std::string label, std::vector<ulong> &position=std::vector<ulong>(), 
                         std::map<std::string, double> &parameters=std::map<std::string, double>());
  */
};
